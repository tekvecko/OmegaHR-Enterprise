...
# Oprava gen_pdf: absolutní cesty, fallback data, Arial bez diakritiky
new_gen_pdf = """
def gen_pdf(token, type='contract'):
    ...
    try:
        ...
        content = f"Token: {token}\\nName: {pd.get('name')} {pd.get('surname')}\\nPosition: {hd.get('position')}\\nSalary: {hd.get('salary')}"
...
# Injekce nové funkce do kódu
import re
code = re.sub(r"def gen_pdf\(.*?\):.*?(@app\.route|if __name__)", new_gen_pdf + "\n\\1", code, flags=re.DOTALL)

with open('omega_core.py', 'w', encoding='utf-8') as f:
    f.write(code)
print("✅ Jádro (omega_core.py) bylo kompletně zrekonstruováno.")
